...
exports.cleanProjects = cleanProjects;
...
async function seedProjects() {
    try {
        logger_1.logger.info('🚀 Starting projects seed...');
        // Load demo projects data
        const projectsData = JSON.parse(fs_1.default.readFileSync(path_1.default.join(process.cwd(), 'prisma/seed/data/demo-projects.json'), 'utf-8'));
        // Clean existing demo projects
        logger_1.logger.info('🧹 Cleaning existing demo projects...');
        ...
        logger_1.logger.info('👤 Creating project profiles for users...');
        for (const user of users) {
            if (!user.projectProfile) {
                try {
                    ...
                    logger_1.logger.warn(`Project profile creation failed for user ${user.email}: ${error}`);
                }
            }
        }
        // Create projects
        logger_1.logger.info('📁 Creating demo projects...');
        const createdProjects = await Promise.all(projectsData.map(async (projectData) => {
            const user = users.find(u => u.email === projectData.authorEmail);
            if (!user) {
                logger_1.logger.warn(`User not found for email: ${projectData.authorEmail}`);
                return null;
            }
            ...
            if (!projectProfile) {
                try {
                    ...
                    if (!projectProfile) {
                        logger_1.logger.error(`Could not create or find project profile for user ${user.email}`);
                        return null;
                    }
                }
            }
        ...
        logger_1.logger.info(`✅ Successfully created ${validProjects.length} demo projects`);
        ...
        logger_1.logger.error('❌ Projects seeding failed:', errorMessage);
        throw errorMessage;
    }
}
async function cleanProjects() {
    try {
        logger_1.logger.info('🧹 Cleaning demo projects...');
        ...
        logger_1.logger.info('✅ Demo projects cleaned successfully');
        console.log('🧹 Demo projects cleaned successfully!');
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error : new Error(String(error));
        logger_1.logger.error('❌ Projects cleanup failed:', errorMessage);
        throw errorMessage;
    }
}
